import React from "react";

/**
 * SAVOLogo — Phase 1 Brand Foundation. THE canonical master logo.
 * ============================================================
 * Geometry source: "Final Master 6D.2" (uploaded 2026-08-15). The path
 * data below is copied byte-for-byte from the approved source file — not
 * redesigned, reconstructed, approximated, or substituted with a font.
 * Only the export style was adapted (named export, matching this
 * project's existing component convention) — that is a packaging
 * change, not a geometry change.
 *
 * currentColor-compatible: when no `variant` is given, the four
 * letterforms (S-A-V-O) inherit `color` from context, so this
 * component works correctly wherever it's dropped without any props.
 * The `variant` prop below gives named presets for the four approved
 * contexts requested in Phase 1 Step 5:
 *   - "primary-dark":  ink letterforms (#0C0F16) — for light backgrounds
 *   - "primary-light": light letterforms (#EEF2FF) — for dark backgrounds
 *   - "mono-white":    fully white, including the Discovery Point
 *   - "mono-black":    fully black, including the Discovery Point
 * An explicit `pointColor` prop always overrides the variant's point
 * color, and the SAVO Discovery token (#00D4A1) is the default point
 * color for "primary-dark"/"primary-light"/no-variant, matching the
 * approved brand spec.
 *
 * This is the ONLY master-logo geometry in this codebase going
 * forward. The pre-existing `src/components/layout/savo-logo.tsx`
 * (the old rounded-square "wavy S" mark) is untouched by this file —
 * per the brief, existing occurrences are not replaced yet in this
 * phase to avoid regressing pages that render it today.
 *
 * Discovery Point motion (SAVO Discovery Spark — approved SAVO motion
 * identity): the point is a separate <path> from the four letterforms
 * specifically so it can move independently — S/A/V/O never animate,
 * only the point does. Pure CSS keyframe animation (see
 * .savo-logo-point in globals.css): an intermittent ~4s cycle — one
 * ~0.7s "spark" (small upward nudge + scale + subtle glow), then back
 * to the exact resting position for the rest of the cycle. No JS/
 * client component needed — this keeps SAVOLogo a server component
 * everywhere it's already used (header, footer). Respects
 * prefers-reduced-motion (falls back to the static resting point).
 * Opt-in via `animated` — defaults to false; currently enabled at the
 * header call site only.
 */
export type SAVOLogoVariant = "primary-dark" | "primary-light" | "mono-white" | "mono-black";

export interface SAVOLogoProps extends React.SVGAttributes<SVGSVGElement> {
  title?: string;
  pointColor?: string;
  variant?: SAVOLogoVariant;
  /** Discovery Point motion on mount. Opt-in — defaults to false (static resting logo). */
  animated?: boolean;
}

const VARIANT_COLORS: Record<SAVOLogoVariant, { letters: string; point: string }> = {
  "primary-dark": { letters: "#0C0F16", point: "#00D4A1" }, // SAVO Ink letterforms on light surfaces
  "primary-light": { letters: "#EEF2FF", point: "#00D4A1" }, // SAVO Text letterforms on dark surfaces
  "mono-white": { letters: "#FFFFFF", point: "#FFFFFF" }, // fully monochrome — Discovery Point included
  "mono-black": { letters: "#0C0F16", point: "#0C0F16" }, // fully monochrome — Discovery Point included
};

export function SAVOLogo({ title = "SAVO", pointColor, variant, animated = false, ...props }: SAVOLogoProps) {
  const preset = variant ? VARIANT_COLORS[variant] : null;
  const lettersColor = preset ? preset.letters : "currentColor";
  const resolvedPointColor = pointColor ?? (preset ? preset.point : "#00D4A1");

  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 316 74" role="img" aria-label={title} {...props}>
      <title>{title}</title>
      <g fill={lettersColor} fillRule="evenodd">
        <path d="M 152 1 L 154 5 L 154 7 L 158 14 L 158 16 L 161 21 L 161 23 L 166 32 L 166 34 L 170 41 L 170 43 L 174 50 L 174 52 L 178 59 L 178 61 L 183 71 L 205 71 L 205 69 L 209 62 L 209 60 L 213 53 L 213 51 L 218 42 L 218 40 L 220 37 L 220 35 L 225 26 L 225 24 L 229 17 L 229 15 L 236 1 L 217 1 L 215 4 L 215 6 L 210 16 L 210 18 L 204 30 L 204 32 L 202 35 L 202 37 L 200 40 L 200 42 L 194 54 L 192 52 L 191 50 L 191 48 L 188 43 L 188 41 L 185 36 L 185 34 L 183 31 L 183 29 L 181 26 L 181 24 L 178 19 L 178 17 L 176 14 L 176 12 L 174 9 L 174 7 L 172 4 L 172 2 L 171 1 Z" />
        <path d="M 108 1 L 105 7 L 105 9 L 97 24 L 97 26 L 90 39 L 90 41 L 82 56 L 82 58 L 77 67 L 77 69 L 76 71 L 94 71 L 95 70 L 97 66 L 97 64 L 103 53 L 103 51 L 108 42 L 108 40 L 114 29 L 114 27 L 120 15 L 122 16 L 122 18 L 125 23 L 125 25 L 129 32 L 129 34 L 135 45 L 135 47 L 140 56 L 140 58 L 142 60 L 142 62 L 146 70 L 147 71 L 166 71 L 151 41 L 151 39 L 143 24 L 143 22 L 135 7 L 135 5 L 133 1 Z" />
        <path d="M 252 5 L 249 8 L 248 8 L 243 13 L 243 14 L 241 16 L 239 20 L 238 25 L 237 26 L 237 29 L 236 30 L 236 42 L 237 43 L 237 46 L 238 47 L 239 52 L 241 54 L 244 60 L 248 64 L 249 64 L 251 66 L 252 66 L 254 68 L 258 70 L 260 70 L 261 71 L 264 71 L 265 72 L 270 72 L 271 73 L 279 73 L 280 72 L 286 72 L 287 71 L 290 71 L 302 65 L 308 59 L 308 58 L 310 56 L 312 52 L 312 50 L 314 46 L 314 42 L 315 41 L 315 31 L 314 30 L 314 26 L 313 25 L 313 22 L 310 16 L 308 14 L 308 13 L 302 7 L 301 7 L 299 5 L 293 2 L 291 2 L 290 1 L 287 1 L 286 0 L 265 0 L 264 1 L 261 1 L 260 2 L 258 2 Z M 267 15 L 269 15 L 270 14 L 281 14 L 282 15 L 284 15 L 288 17 L 293 22 L 295 26 L 295 28 L 296 29 L 296 32 L 297 33 L 297 39 L 296 40 L 296 44 L 295 45 L 295 47 L 293 49 L 293 50 L 288 55 L 284 57 L 281 57 L 280 58 L 271 58 L 270 57 L 267 57 L 263 54 L 262 54 L 258 50 L 255 44 L 255 39 L 254 38 L 254 33 L 255 32 L 255 28 L 258 22 L 262 18 L 263 18 Z" />
        <path d="M 11 5 L 4 12 L 4 14 L 2 18 L 2 24 L 3 25 L 3 28 L 6 32 L 6 33 L 11 37 L 13 38 L 15 38 L 16 39 L 18 39 L 19 40 L 22 40 L 23 41 L 29 41 L 30 42 L 37 42 L 38 43 L 43 43 L 44 44 L 48 44 L 52 46 L 54 48 L 54 54 L 51 57 L 49 58 L 47 58 L 46 59 L 30 59 L 29 58 L 24 57 L 20 53 L 18 48 L 0 48 L 1 55 L 4 61 L 8 65 L 9 65 L 11 67 L 17 70 L 19 70 L 20 71 L 24 71 L 25 72 L 32 72 L 33 73 L 43 73 L 44 72 L 50 72 L 51 71 L 55 71 L 65 66 L 69 62 L 72 56 L 72 44 L 69 38 L 64 34 L 62 33 L 60 33 L 59 32 L 57 32 L 56 31 L 54 31 L 53 30 L 48 30 L 47 29 L 42 29 L 41 28 L 34 28 L 33 27 L 28 27 L 27 26 L 25 26 L 21 23 L 20 21 L 20 19 L 21 17 L 25 14 L 27 14 L 28 13 L 34 13 L 35 12 L 37 12 L 38 13 L 44 13 L 45 14 L 47 14 L 49 15 L 52 18 L 53 20 L 53 22 L 71 22 L 71 19 L 70 18 L 70 16 L 69 15 L 69 13 L 68 11 L 63 6 L 62 6 L 60 4 L 58 3 L 56 3 L 53 1 L 49 1 L 48 0 L 25 0 L 24 1 L 20 1 L 19 2 L 17 2 Z" />
      </g>
      <path
        d="M 119 44 L 111 49 L 110 51 L 110 60 L 115 64 L 119 66 L 122 66 L 130 61 L 130 49 L 124 46 L 122 44 Z"
        fill={resolvedPointColor}
        fillRule="evenodd"
        className={animated ? "savo-logo-point" : undefined}
      />
    </svg>
  );
}
