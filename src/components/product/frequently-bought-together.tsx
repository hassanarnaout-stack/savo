...
export function FrequentlyBoughtTogether({ items, locale, title, addBundleLabel }: { items: ProductCardData[]; locale: string; title: string; addBundleLabel: string }) {
  ...
  function addBundle() {
    ...
    toast.success(addBundleLabel);
  }

  return (
    <section className="savo-pdp-fbt">
      <div className="savo-pdp-fbt-shell">
        <header className="savo-pdp-fbt-head"><p>BETTER TOGETHER</p><h2>{title}</h2></header>
        <div className="savo-pdp-fbt-panel">
          ...
          <div className="savo-pdp-fbt-summary"><small>{selected.size} selected</small><strong>{formatKWD(total)}</strong><button onClick={addBundle}>{addBundleLabel}</button></div>
        </div>
      </div>
    </section>
  );
